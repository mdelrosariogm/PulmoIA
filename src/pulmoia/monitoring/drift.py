...
def _load_reference_and_current(current: str):
    ...
    if current == "test":
        ...
        scaled = pd.DataFrame(
            scaler.transform(tr[feats127].fillna(tr[feats127].median())), columns=feats127
        )
        cur = scaled[relieff]
...
def main():
    ...
    log.info(
        "Referencia (train)=%d filas | Actual (%s)=%d filas | features=%d",
        len(ref),
        args.current,
        len(cur),
        len(feats),
    ...
    log.info(
        "Drift: %d significativo, %d moderado, %d sin drift (de %d features)",
        n_sig,
        n_mod,
        len(res) - n_sig - n_mod,
        len(res),
    )
    log.info("Top-10 features con más drift:\n%s", res.head(10).to_string(index=False))

    # Gráfico top-20 PSI
    top = res.head(20)
    fig, ax = plt.subplots(figsize=(8, 8))
    colors = {"significativo": "#dc2626", "moderado": "#f59e0b", "sin_drift": "#22c55e"}
    ax.barh(top.feature[::-1], top.psi[::-1], color=[colors[d] for d in top.drift[::-1]])
    ...
    verdict = (
        "ALERTA: drift significativo de datos"
        if n_sig >= max(3, len(feats) // 5)
