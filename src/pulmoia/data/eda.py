...
def load_db(key: str) -> pd.DataFrame | None:
    ...
    if not path or not Path(path).exists():
        ...
        return None
    df = pd.read_csv(path)
    log.info("  %s cargado: %d filas x %d columnas", key.upper(), len(df), len(df.columns))
    return df
...
def console_stats(df: pd.DataFrame, name: str):
    ...
    if "source" in df.columns:
        ...
        for src, cnt in df["source"].value_counts().items():
            log.info("  %-10s: %6d ventanas (%.1f%%)", src, cnt, cnt / len(df) * 100)
    ...
    if "diagnosis" in df.columns:
        ...
        for dx, cnt in df["diagnosis"].value_counts().items():
            log.info("  %-8s: %6d ventanas (%.1f%%)", dx, cnt, cnt / len(df) * 100)
    ...
    if available:
        ...
        for t in available:
            ...
            pct = n / len(df) * 100
            bar = "#" * int(pct / 2)
            log.info("  %-15s: %6d / %d (%.1f%%) |%s", t, n, len(df), pct, bar)
        ...
        for combo, cnt in combos.value_counts().head(10).items():
            parts = combo.split("-")
            active = [
                t.replace("has_", "") for t, p in zip(available, parts, strict=False) if p == "1"
            ]
            label = "+".join(active) if active else "normal"
            log.info("  %-30s: %6d (%.1f%%)", label, cnt, cnt / len(df) * 100)
        ...
        for n, cnt in df_tmp["n_labels"].value_counts().sort_index().items():
            ...
            log.info("  %-12s: %6d (%.1f%%)", label, cnt, cnt / len(df) * 100)
    ...
    for feat, row in stats.iterrows():
        log.info(
            "  %-40s mean=%8.3f std=%8.3f min=%8.3f max=%8.3f",
            feat[:40],
            row["mean"],
            row["std"],
            row["min"],
...
def plot_label_distribution(df: pd.DataFrame, name: str):
    ...
    for bar, t in zip(bars, available, strict=False):
        ax.text(
    ...
    for c in top_combos.index:
        parts = c.split("-")
        active = [t.replace("has_", "") for t, p in zip(available, parts, strict=False) if p == "1"]
    ...
    xlabels = [
        f"{int(n)} evento{'s' if n>1 else ''}" if n > 0 else "normal" for n in n_counts.index
    ]
    ax.bar(xlabels, n_counts.values / n_total * 100, color="#4C72B0", alpha=0.85, edgecolor="white")
    ax.set_ylabel("% ventanas")
    ax.set_title("Etiquetas activas por ventana")
    for i, (_, v) in enumerate(n_counts.items()):
        ax.text(i, v / n_total * 100 + 0.3, f"{v:,}", ha="center", fontsize=8)
...
def plot_feature_distributions(df: pd.DataFrame, name: str):
    ...
    for ax, feat in zip(axes.flat, sample, strict=False):
        ax.hist(df[feat].dropna(), bins=50, alpha=0.4, color="gray", label="todos", density=True)
...
def plot_comparison(dfs: dict):
    ...
    fig.suptitle("Comparacion de distribuciones entre bases de datos", fontsize=13)

    for ax, feat in zip(axes.flat, key_features, strict=False):
    ...
    if data_plot:
        ...
        ax.set_title("Prevalencia de eventos acusticos por base de datos", fontsize=12)
        ax.set_ylabel("% ventanas positivas")
...
def generate_sweetviz(
    ...
    meta_keep = [
        c for c in ["source", "equipment", "location", "diagnosis", "t_start_s"] if c in df.columns
    ]
    ...
    try:
        ...
        if compare_df is not None:
            ...
            df_compare = compare_df[[c for c in cols_report if c in compare_df.columns]].copy()
...
def main(db_filter: str | None):
    ...
    if db_filter is None or db_filter == "combined":
        parts = [v for k, v in dfs.items() if k in ("HF", "ICBHI") and v is not None]
        if parts:
    ...
    if dfs.get("HF") is not None and dfs.get("ICBHI") is not None:
        ...
        generate_sweetviz(
            dfs["HF"], "HF", target="has_crackle", compare_df=dfs["ICBHI"], compare_name="ICBHI"
        )

    # Comparación gráfica entre DBs
    plot_comparison({k: v for k, v in dfs.items() if k in ("HF", "ICBHI", "TR") and v is not None})
